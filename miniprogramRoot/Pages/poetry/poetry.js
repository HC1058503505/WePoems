...
Page({
  ...
  requestMe: function() {
    ...
    let poetryjson = wx.getStorageSync("poetryjson")
    let categorysearch = wx.getStorageSync("categorysearch")
    let urlPath = categorysearch == "mingju" ? "/api/mingju/juv11.aspx" : "/api/shiwen/shiwenv11.aspx"
    let postData = {
      "token": "gswapi",
      "id": poetryjson
    }
  ...
  authorMorePoems: function() {
    ...
    wx.setStorage({
      data: this.data.poetryinfo.tb_author.nameStr,
      key: 'AuthorName',
    });
  ...
  requestAuthorMore: function() {
    ...
    return new Promise((reslove, reject) => {
      wx.request({
        url: app.globalData.baseURL + '/api/author/author10.aspx',
        method: 'POST',
        data: postData,
        header: {
          'content-type': 'application/x-www-form-urlencoded'
